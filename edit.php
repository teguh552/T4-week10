...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!empty($judul) && !empty($pengarang) && !empty($penerbit) && !empty($tahun_terbit) && !empty($stok)) {
        $stmt = $pdo->prepare("UPDATE buku SET judul = :judul, pengarang = :pengarang, penerbit = :penerbit, tahun_terbit = :tahun_terbit, stok = :stok WHERE id = :id");
        $stmt->execute([
            ':judul' => $judul,
            ':pengarang' => $pengarang,
            ':penerbit' => $penerbit,
    ...
    <div class="container mt-5" style="max-width: 600px;">
        ...
        <form method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="number" name="tahun_terbit" class="form-control" value="<?= htmlspecialchars($buku['tahun_terbit']) ?>" required>
            </div>
            <div class="mb-3">
